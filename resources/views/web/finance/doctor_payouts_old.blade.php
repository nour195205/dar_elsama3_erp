...
<form action="{{ route('finance.doctor_payouts.mark') }}" method="POST">
    @csrf
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        @if(!$p->is_paid)
                            <input type="checkbox" name="payout_ids[]" value="{{ $p->id }}" class="payout-checkbox">
                        @else
                    ...
                    <td>
                        @if(!$p->is_paid)
                            <form action="{{ route('finance.doctor_payouts.mark_individual', $p->id) }}" method="POST" style="display: inline;">
                                @csrf
                                <button type="submit" class="btn-sm" style="background: none; border: none; color: #00e699; cursor: pointer; font-size: 0.9rem;" title="تأكيد الدفع">
    ...
    <div style="margin-top: 1.5rem; display: flex; justify-content: flex-end;">
        <button type="submit" class="btn btn-primary" id="markPaidBtn" style="display: none; background: #00e699; color: #111827; font-weight: bold;">
...
@endsection
        const anyChecked = Array.from(document.querySelectorAll('.payout-checkbox')).some(cb => cb.checked);
        document.getElementById('markPaidBtn').style.display = anyChecked ? 'flex' : 'none';
    }
</script>
@endsection
