...
<div class="table-container" style="max-width: 720px; margin: 0 auto;">
    <h2 style="margin-bottom: 1rem;">تعديل المجموعة</h2>
    <form action="{{ route('permission-groups.update', $group) }}" method="POST">
        @csrf
        @method('PUT')
        <div style="display:grid;gap:1.25rem;margin-bottom:1.5rem;">
            <div>
                ...
                <input type="text" name="name" required value="{{ old('name', $group->name) }}" style="width:100%;padding:0.9rem;border-radius:12px;background:rgba(255,255,255,0.05);border:1px solid var(--border-color);color:#fff;">
            </div>
            <div>
                <label style="display:block;margin-bottom:0.35rem;color:var(--text-muted);">وصف مختصر</label>
                <input type="text" name="description" value="{{ old('description', $group->description) }}" style="width:100%;padding:0.9rem;border-radius:12px;background:rgba(255,255,255,0.05);border:1px solid var(--border-color);color:#fff;">
            </div>
            <div>
                ...
                <div style="display:grid;gap:0.5rem;max-height:320px;overflow-y:auto;padding:0.5rem;border-radius:12px;border:1px solid var(--border-color);">
                    ...
                    <label style="display:flex;gap:0.6rem;align-items:flex-start;cursor:pointer;font-size:0.9rem;">
                        <input type="checkbox" name="permission_ids[]" value="{{ $perm->id }}"
                            {{ $group->permissions->contains('id', $perm->id) ? 'checked' : '' }}
        ...
        <div style="display:flex;gap:1rem;justify-content:flex-end;">
            <a href="{{ route('permission-groups.index') }}" class="btn" style="background:rgba(255,255,255,0.06);color:#fff;">رجوع</a>
